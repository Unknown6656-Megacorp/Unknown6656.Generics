...
public static class StringExtensions
{
     ...
     *  
     *  ouptut: 𝐚𝐛𝐜𝐝𝐞𝐟𝐠𝐡𝐢𝐣𝐤𝐥𝐦𝐧𝐨𝐩𝐪𝐫𝐬𝐭𝐮𝐯𝐰𝐱𝐲𝐳𝐀𝐁𝐂𝐃𝐄𝐅𝐆𝐇𝐈𝐉𝐊𝐋𝐌𝐍𝐎𝐏𝐐𝐑𝐒𝐓𝐔𝐕𝐖𝐗𝐘𝐙𝟎𝟏𝟐𝟑𝟒𝟓𝟔𝟕𝟖𝟗
    ...
    {
        string esc = @string.Select(GetLiteral).StringConcat();

        return include_quotes ? $"\"{esc}\"" : esc;
    }

    internal static string GetLiteral(char character) => character switch
    {
        '\"' => @"\""",
        '\\' => @"\\",
        '\0' => @"\0",
    ...
    public static string RemoveDiacritics(this string text, bool german = true)
    {
        text = text.Replace("ß", "ss")
                   .Replace("ẞ", "Ss"); // todo : detect if 'ẞ' should be mapped to 'SS' or 'Ss'
        ...
        for (int i = 0; i < normalized.Length; i++)
            if (CharUnicodeInfo.GetUnicodeCategory(normalized[i]) is not UnicodeCategory.NonSpacingMark)
